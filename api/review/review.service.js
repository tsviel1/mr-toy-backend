...
async function query(filterBy = {}) {
  ...
  const reviews = await collection
    .aggregate([
      ...
      {
        $lookup: {
          from: 'user', // from which collection?
          foreignField: '_id', // which field in the other collection?
          localField: 'userId', // what proprety in "review"?
          as: 'user', // name of new key, 
        },
      },
      { $unwind: '$user' }, // [{.....}] ==> {.....}
      {
      ...
      { $unwind: '$toy' }, // [{.....}] ==> {.....}
      {
        $project: { // מתאר את האובייקט הסופי שלי
          _id: 1,
          content: 1,
          rate: 1,
          user: { _id: 1, username: 1 }, // give me just the _id and username from "user" collction.
          toy: { _id: 1, name: 1, price: 1 }, // give me just the _id,... from "toy" collction.
        },
      },
    ])
    .toArray()
